...
| Go | 1.22+ | Worker de ingestión (opcional) |
| Rust | Latest | clamo-sync y dashboards (opcional) |
| Docker | Latest | Servicios contenerizados |
...
|----------|-------------|------|
| `clamo-web` | Frontend Next.js (puerto 3001) | Local |
| `clamo-cases` | API de casos (puerto 4000) | Local |
| `clamo-tenant` | API de tenants (puerto 4001) | Local |
| `clamo-dashboards` | API de dashboards (Rust) | Local |
| `clamo-widget-server` | Servidor de widgets | Local |
| `luna` | Asistente IA unificado (Javi, Clamy, Luna) | Local |
| `clamo-tools` | Servidor de herramientas Olive | Local |
| `clamo-cej-connector` | Conector CEJ | Docker |
| `clamo-cej-discovery` | Workers de descubrimiento | Docker |
| `clamo-ingest-go` | Worker de ingesta | Docker |

<Note>
**Nota sobre Asistentes IA:** `javi` y `clamy` ya no son servicios separados. Ahora son **perfiles** (tool profiles) dentro del servicio unificado `luna` (desplegado como `ask-clamo`).
</Note>
...
Todos los servicios tienen recarga en caliente habilitada:

| Servicio | Recarga en Caliente |
|----------|------------|
...
| clamo-cej-connector (Docker) | Vía montaje de volúmenes |

<Warning>
Cambios en `clamo-commons` requieren reconstrucción manual:
```bash
...
- Cambios en código Python (recarga en caliente)
- Cambios en `.env`
...
<AccordionGroup>
  ...
  <Accordion title="Puerto ya en uso">
    ...
    ```bash
    lsof -i :3001  # Para web app
    lsof -i :4000  # Para clamo-cases
    lsof -i :4001  # Para clamo-tenant
    ```
    Mata el proceso o cambia el puerto en tu configuración.
  </Accordion>
</AccordionGroup>
