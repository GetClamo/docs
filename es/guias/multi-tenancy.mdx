...
Cada request resuelve dinámicamente la conexión del tenant basándose en los headers `x-workos-*`:

```typescript
// Middleware de resolución
async function resolveTenant(c: Context, next: Next) {
  // 1. Extraer companyId del contexto de sesión (headers x-workos-*)
  ...
  // 2. Resolver companyId desde organizationId de WorkOS
  const company = await controlPrisma.company.findFirst({
    where: { workosOrganizationId: session.orgId },
  ...
  const connectionUri = await vault.getSecret(company.Database.pooledConnSecretId);
