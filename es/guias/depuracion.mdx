...
| clamo-cej-connector | Docker logs | JSON |
| clamo-ingest-go | Docker logs | Structured |

<Note>
**Temporal UI:** Si ejecutas clamo-cej-connector con Docker Compose, Temporal UI está disponible en http://localhost:8233. Esto es parte del stack de Docker, no un servicio separado.
</Note>
...
# Obtener usuario actual (requiere headers x-workos-*)
curl http://localhost:4001/v1/me \
  -H "Authorization: Bearer $TOKEN" \
  -H "x-workos-user-id: $USER_ID" \
  -H "x-workos-org-id: $ORG_ID" | jq
...
### Generar Headers para Testing

En desarrollo local sin Kong, necesitas proporcionar los headers `x-workos-*` manualmente:

```bash
# Exportar variables
export TOKEN="tu_access_token_de_workos"
export USER_ID="user_01HQMK..."
export ORG_ID="org_01HQML..."

# Usar en requests
curl http://localhost:4000/v1/cases \
  -H "Authorization: Bearer $TOKEN" \
  -H "x-workos-user-id: $USER_ID" \
  -H "x-workos-org-id: $ORG_ID"
```

## Depuración de Temporal Workflows

<Note>
**Nota:** Temporal solo está disponible cuando ejecutas clamo-cej-connector con Docker Compose. No es un servicio que se ejecute de forma independiente en desarrollo local.
</Note>

### Temporal Web UI

Si tienes el stack de Docker corriendo, accede a http://localhost:8233 para:
