...
<Warning>
**Status: In Development** - Webhook functionality is planned but not yet fully implemented. This documentation describes the intended design. Currently, real-time notifications are handled through **Ably** for case events.
</Warning>

Webhooks will allow your application to receive notifications when important events occur in Clamo.

## Current Notifications (Ably)

Currently, Clamo uses **Ably** for real-time notifications of case events:

```typescript
// clamo-web-app - Subscribe to case events
...
channel.subscribe('case.updated', (message) => {
  console.log('Case updated:', message.data);
});

channel.subscribe('movement.created', (message) => {
  console.log('New movement:', message.data);
});
```
...
<Note>
The following section describes the planned API for webhooks. This functionality is not yet available.
</Note>
