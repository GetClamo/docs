...
### Llamadas desde el Asistente IA

El asistente IA unificado (luna/ask-clamo) propaga headers cuando llama a APIs backend a través de clamo-tools:

```python
# luna/src/luna/tools/olive_client.py
async def call_olive_tool(tool_name: str, ctx: AppContext) -> dict:
    # Los headers se construyen desde el contexto de sesión
    headers = None
    if ctx.session and ctx.session.principal_type == "user":
        headers = {
            "x-workos-user-id": ctx.session.user_id,
            "x-workos-org-id": ctx.session.org_id,
        }
    
    return await olive_client.call_tool(
        tool_name,
        {
            "case_id": ctx.case_id,
